...
from Test.agent.JSP.End2End_agent import Agent
from Test.optimizer.JSP_optimizer.JSP_RL_algorithm.End2End_optimizer import WrapperRay
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir+f"/{experiment_name}")
        ...
        while len(to_worker) < num_envs:
            with torch.no_grad() as a:
                all_actions, logprobas, _ = agent(next_interval_reps, next_attention_interval_masks,
                                                  next_job_resource_masks, next_mask, next_indexes,
                                                  next_start_end_tokens)
            ...
            for env_idx, info in enumerate(infos):
                ...
                if 'makespan' in info.keys():
                    ...
                    if len(temporary_action_history[env_idx]) > 0:
                        to_worker.append((fn_env[env_idx], list(temporary_action_history[env_idx]),
                                          json.loads(info['solution']), makespan))
