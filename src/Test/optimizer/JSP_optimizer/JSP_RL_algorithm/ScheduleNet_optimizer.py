...
from Test.agent.JSP.ScheduleNet_agent import ScheduleNet_agent
...
class ScheduleNet_optimizer(Basic_learning_algorithm):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

    def rollout(self, data, config, dev, embedding_net=None, policy_net=None, verbose=True):
        s = ScheduleNet_agent(config, data)
        s.reset()
        done = False
        ...
        while True:
            ...
            if all_machine_work:  # all machines are on processing. keep process!
                ...
                if embedding_net is not None and \
                    ...
                    idle_machine, doable_op = list(idle_machine_and_its_doable_ops.keys()), list(
                        idle_machine_and_its_doable_ops.values())
                    ...
                    embedded_g = embedding_net(**input_graphs)
                    op_id, _ = policy_net(idle_machine, doable_op, embedded_g)
    ...
    def update(self, data, config):
        ...
        _, t, min_makespan = self.rollout(data, config, dev, tga, policy, False)
        t2=time.time()
