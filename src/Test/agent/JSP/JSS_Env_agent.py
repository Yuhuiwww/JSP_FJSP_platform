...
class JssEnv(gym.Env):
    def __init__(self,config):
        ...
        self.jobs = config.Pn_j
        self.machines = config.Pn_m
        ...
        for i in range(self.jobs):
            self.max_time_op=np.max(problem.data[0])
            for j in range(self.machines):
               self.jobs_length[i] += problem.data[0][j]
               self.sum_op += problem.data[0][j]
               self.instance_matrix[i][j]=([problem.data[1][i][j]-1, problem.data[0][i][j]])
...
class JSS_Env_agent(Basic_Agent):
    def __init__(self,data,config):
        ...
        for i in range(self.jobs):
            self.max_time_op = np.max(data[0])
            for j in range(self.machines):
                self.jobs_length[i] += data[0][i][j]
                self.sum_op += data[0][i][j]
                self.instance_matrix[i][j] = ([data[1][i][j] - 1, data[0][i][j]])
        self.max_time_jobs = max(self.jobs_length)
    ...
    def _is_done(self):
        if self.nb_legal_actions == 0:
            ...
            # print('makespan',self.current_time_step)
            return True,self.current_time_step
        return False,0
